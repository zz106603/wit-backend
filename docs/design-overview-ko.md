...
- Google Calendar 연동
- 향후 7일 이내 일정 중 시작 시간이 빠른 순서로 최대 3개 조회
- 날씨 예보 기반 외출 준비 목적에 맞게 7일 밖 일정은 부족한 개수만큼 채우지 않음
...
- Google Calendar location을 우선 사용하고, 비어 있으면 지역명/상호명/주소성 표현이나 직접적 장소 지시어가 있는 title만 보조 후보로 사용
...
- 일정 시작 시점
- 일정 종료 시점
- 원래 의도는 현재 위치와 목적지 날씨 비교
- 실제 현재 위치가 없으면 currentWeather를 임의로 대체하지 않고 start/end 기준으로만 추천
