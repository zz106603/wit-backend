...
일정 조회는 날씨 기반 외출 준비 목적에 맞춰
`현재 시각 ~ 7일 이내` 일정 중 시작 시간이 빠른 순서로 최대 3개만 사용한다.
...
- rawLocation
  ...
  - 허용: 지역명, 상호명, 주소성 표현, 직접적인 장소 지시어(예: `강남역`, `판교 카페`, `테헤란로`, `회사`, `사무실`, `집`, `학교`)
  - 비허용: 일반 활동명이나 추상 표현(예: `회의`, `운동`, `약속`, `공부`, `점심`, `저녁`, `회식`)
  - title에서 모호하거나 추상적인 표현만 보이면 위치를 추론하지 않는다
```
